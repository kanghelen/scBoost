...
parser = argparse.ArgumentParser(description="Single-method XGBoost baseline for scRNA-seq embeddings")
...
parser.add_argument("--method", required=True, help="Single obsm key for the embedding method")
...
print(f"=== Single-method XGBoost: {method} ===")
print(f"  Embedding dim: {adata.obsm[method].shape[1]}")
...
# Train & predict (ConcatXGBoost with a single method = single-method baseline)
clf = ConcatXGBoost(obsm_keys=[method], label_key="cell_type", batch_key="batch",
                    use_batch_feature=not args.no_batch_feature, device=device)
clf.fit(train)
clf.predict(test, key_added="single_pred")

# --- Evaluate on test set ---
y_true = np.asarray(test.obs["cell_type"])
y_pred = np.asarray(test.obs["single_pred"])
...
tag = f"{args.sample_name}_single_{method}_{split_tag}"
report_df.to_csv(os.path.join(out_dir, f"{tag}_classification_report.tsv"), sep="\t", index_label="cell_type")
ts = datetime.now().strftime("%y%m%d_%H%M%S")
joblib.dump(clf, os.path.join(out_dir, f"{ts}_{tag}_single_xgboost.pkl"))
print(f"\nSaved to {out_dir}/")
